...
> Все функции работают через `fetch()` к соответствующим эндпоинтам и обновляют DOM-вёрстку внутри `#content`.  
> Не забудьте перед запуском убедиться, что сервер (`server.py`) запущен и рубрика `API_URL` в `client.html` указывает на `http://127.0.0.1:5000`.  
